...
# push_with_retry (transient-drop hardening) and assert_platform (empty-arch
# guard) now live in nix-push-lib.sh so nix-publish-base.sh gets exactly the same
# behaviour instead of a second copy that drifts. See that file for the reasoning
# behind each; PUSH_ATTEMPTS=1 still restores fail-fast pushing.
PUSH_LOG_PREFIX="[nix-publish]"
# shellcheck source=ci-scripts/nix-push-lib.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/nix-push-lib.sh"
