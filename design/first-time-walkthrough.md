...
1. Head over to GitLab pipelines <https://gitlab.com/kasm-technologies/labs-sandbox/kasm-nix/-/pipelines> and find a Passed and completed run.

2. Click the download dropdown next to the row and download the `publish` job's build artifacts.

3. There should be 2 artifacts `nix-build-report.md` and `nix-build-report.json` that contain details about what was built and pushed during the run.