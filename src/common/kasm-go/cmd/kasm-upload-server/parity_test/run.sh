...
KG_DIR="${REPO_ROOT}/src/common/kasm-go"
GO_BIN_HOST="${KG_DIR}/bin/kasm-upload-server.linux-arm64"
GO_BIN_ALT="${KG_DIR}/bin/kasm-upload-server.linux-amd64"
...
if [[ ! -x "${GO_BIN}" ]]; then
  ...
  yellow "Run: make -C ${KG_DIR} upload"
  exit 2
fi
