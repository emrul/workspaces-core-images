...
| `publish` | `publish` | inside the store: `podman login` the registry, then `ci-scripts/nix-publish.sh` tags each to its kasm name and pushes to `$REGISTRY_NS` (scoped to `NIX_PROFILES`). Records each image's `uncompressedBytes` in `nix-build-report.json` and exports `PUBLISH_JOB_ID`. |
...
| `scan` | `refresh-registry` | Triggers the `kasm-nix-registry` pipeline (job-token auth, `strategy: depend`, `allow_failure`) passing `SECURITY_JOB_ID` **and `SIZES_JOB_ID`** (the `publish` job, for the per-workspace uncompressed sizes), so the Pages site is rebuilt around the new `security.json` and size map. Pages deployments are immutable — republishing is the only way to update either. Both are best-effort: an absent id means the registry carries forward what it already serves. |
